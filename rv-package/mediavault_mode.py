...
try:
    ...
    _HAS_GL = True
    # GL 2.0 shader functions via ctypes  — PyOpenGL's wrappers crash
    # in RV with "invalid index to scalar variable" because
    # glGetIntegerv returns a scalar instead of an array.
    # ctypes calls the raw C API correctly.
    # NOTE: Deferred to first render() call because we need an active
    # GL context for wglGetProcAddress.  At import time, no context.
    print("[MediaVault] GL init: HAS_GL=%s  (shader init deferred to first render)" % _HAS_GL)
except ImportError:
    pass
...
class MediaVaultMode(rv.rvtypes.MinorMode):
    ...
    def _setLUTOnSourceGroup(self, sg, lut_file, lut_name):
        ...
        try:
            ...
            # Set the global OCIO env var so RV's internal OCIO
            # subsystem can find ACEScg, ACEScct, etc. without
            # per-node config.  This eliminates the
            # "OCIO environment variable not set" warning and
            # ensures Display nodes can resolve color spaces.
            os.environ['OCIO'] = config_path
            print("[LUT] Set OCIO env var: %s" % config_path)
            ...
            #
            # ocioUpdateConfig() may clear/reset properties that we
            # added (outColorSpace is not a standard OCIOLook schema
            # property).  Strategy:
            #   a) Disable the node
            #   b) Create missing properties
            #   c) Set ALL values
            #   d) ocioUpdateConfig (loads config, may clear extras)
            #   e) Re-set outColorSpace if ocioUpdateConfig wiped it
            #   f) Re-enable
            ...
            # 5d — Load config (may reset outColorSpace)
            try:
            ...
            # 5e — Re-set outColorSpace in case ocioUpdateConfig
            #      cleared it during internal re-init.
            try:
                cur = rvc.getStringProperty(
                    ocio_look + ".ocio.outColorSpace")
                if not cur or not cur[0]:
                    print("[LUT] outColorSpace was cleared — re-setting")
                    rvc.setStringProperty(
                        ocio_look + ".ocio.outColorSpace",
                        ["ACEScg"], True)
            except Exception:
            ...
            try:
                for disp_group in rvc.nodesOfType("RVDisplayPipelineGroup"):
                    ...
                    if ocio_disp:
                        # With OCIO env var set, the display node
                        # already has access to the full ACES config.
                        # Just set our desired display/view.
    ...
    def render(self, event):
        ...
        # ── Lazy init: resolve GL 2.0 functions via ctypes ──
        # Must be done here (not at import time) because we need an
        # active GL context for wglGetProcAddress.
        if not _HAS_GL_SHADERS and not getattr(self, '_gl_ctypes_tried', False):
            ...
            if sys.platform == 'win32':
                ...
                # macOS/Linux: try PyOpenGL as fallback
                try:
                    from OpenGL.GL import glUseProgram as _fn_use
                    from OpenGL.GL import glGetIntegerv as _fn_getiv
                    _glUseProgram = _fn_use
                    _glGetIntegerv_ct = None  # use PyOpenGL on non-Windows
                    _HAS_GL_SHADERS = True
                except ImportError:
                    pass
        ...
        _do_diag = (_diag_count < 5)

        if _do_diag and _diag_count == 0:
            print("[OVERLAY-DIAG] === FIRST RENDER ===")
            print("[OVERLAY-DIAG] _HAS_GL_SHADERS=%s  _glUseProgram=%s  _glGetIntegerv_ct=%s" %
                  (_HAS_GL_SHADERS, type(_glUseProgram).__name__, type(_glGetIntegerv_ct).__name__))
