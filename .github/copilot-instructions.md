...
│   ├── mediavault_mode.py        # Full Qt asset picker + menus + overlay system + OCIO LUT pipeline (4697 lines)
...
A ~4,700-line Python plugin that adds a **MediaVault** menu, **OpenGL overlay system**, **ComfyUI metadata overlay**, and **per-shot OCIO LUT auto-application** to OpenRV:
...
### Per-Shot LUT Auto-Application (OCIO Pipeline)

When an EXR or media file is loaded, the plugin automatically queries the CAM server for a matching LUT and applies it via OCIO. This replicates the exact same color pipeline as Nuke's `OCIOFileTransform` with `working_space='color_timing'`.

**Problem Solved**: .cube LUTs from DaVinci Resolve expect ACEScct (log) input. RV's native `readLUT()` feeds raw linear ACES pixels into the cube, producing wrong colors. Six different approaches to work around this failed (baked shapers, DOMAIN_MAX, .lut.scale, 1D+3D cubes). The solution uses OCIO's built-in colorspace management.

**Architecture:**
```
Source loaded → _onSourceLoaded → _applyProjectLUTs()
  → _fetchLUTForPath(filepath)          ← HTTP GET /api/assets/lut-for-path
  → resolve disk path / download LUT
  → _setLUTOnSourceGroup(sg, lut_file)  ← OCIO pipeline swap
    → _getOrCreateOCIOConfig(cube_path) ← builds OCIO config with Look
    → swap RVLookPipelineGroup: RVLookLUT → OCIOLook
    → set ocio.config, ocio.function="look", ocio_look.look="shot_grade"
```

**Transform Chain (automatic via OCIO):**
`ACEScg → ACEScct → apply .cube → ACEScct → ACEScg`

**Key Functions:**
| Function | Purpose |
|----------|--------|
| `_getOrCreateOCIOConfig(cube_path)` | Creates OCIO config from built-in ACES studio config + adds Look with `processSpace='ACEScct'` + FileTransform pointing to .cube. Cached per cube hash in temp dir. |
| `_setLUTOnSourceGroup(sg, lut_file, lut_name)` | Swaps `RVLookPipelineGroup` pipeline from `RVLookLUT` to `OCIOLook`, sets OCIO properties, calls `ocioUpdateConfig()` |
| `_applyProjectLUTs()` | Iterates source groups, calls `_fetchLUTForPath()`, resolves cross-platform paths, downloads if needed |
| `_fetchLUTForPath(filepath)` | HTTP GET to `GET /api/assets/lut-for-path?path=<path>` |
| `_findNodeInSG(sg, node_type)` | Recursive node finder — searches pipeline subgroups |

**OCIO Config Details:**
- Built from: `studio-config-v2.1.0_aces-v1.3_ocio-v2.3` (built into OpenRV's PyOpenColorIO v2.3.2)
- Key colorspaces: `ACEScg` (scene_linear), `ACEScct` (color_timing)
- Look name: `shot_grade` with `processSpace='ACEScct'`
- FileTransform uses `INTERP_TETRAHEDRAL` interpolation
- Config serialized to `%TEMP%/cam_ocio_<md5>.ocio` (cached)

**LUT Overlay**: When a LUT is applied, the `Shift+O` metadata overlay shows `LUT: filename.cube` in the burn-in. Tracked via `_lut_name_by_sg` dict.

**Server-Side LUT Resolution** (`GET /api/assets/lut-for-path`):
1. **Shot-level**: Scans project's `lut_folder` for files containing the shot name/code. Prefers `.cube` over `.cdl`. Uses word-boundary matching with zero-padded variants.
2. **Show-level fallback**: Looks up `project_luts` table by media category (exr/video/image).

**`_findShotLUT(folder, shotName, shotCode)`** in `assetRoutes.js`:
- Scans LUT folder files, scores by: boundary match (100pts) + extension priority
- Zero-pads shot numbers: "10" also tries "0010", "010", "00010"
- Extension priority: `.cube` > `.cdl` > `.3dl` > `.lut` > `.csp` > `.spi1d` > `.spi3d`

**Project Configuration**: `projects.lut_folder` column stores the path to the project's LUT directory. Set in Edit Project modal. Cross-platform resolved via `pathResolver.js`.

**⚠️ CRITICAL — Do NOT revert to native readLUT()**: Six approaches using RV's native LUT pipeline all failed because `readLUT()` has no concept of a working colorspace. `DOMAIN_MAX` is ignored, `.lut.scale` behaves unpredictably, 1D shaper sections are silently dropped. The OCIO Look approach is the only correct solution.
...
| `/api/assets/lut-for-path` | GET | Shot/show LUT lookup by file path (for RV plugin auto-LUT) |
...
| — | **v1.8.x — Per-Shot LUT System + OCIO Pipeline (March 2026)** |
| — | feat: `projects.lut_folder` column — per-project LUT folder path for shot-level grading |
| — | feat: `GET /api/assets/lut-for-path` — shot-level LUT lookup with token scoring + zero-pad variants |
| — | feat: `_findShotLUT()` in assetRoutes.js — folder scan with boundary matching and extension priority |
| — | feat: RV plugin auto-applies per-shot LUTs on source load via `_applyProjectLUTs()` |
| — | feat: LUT name shown in Shift+O overlay burn-in (`LUT: filename.cube`) |
| — | fix: 6 failed native LUT approaches (baked shaper, DOMAIN_MAX, .lut.scale, 1D+3D, etc.) |
| `b09b343` | feat: OCIO Look pipeline — replaces all baked cube attempts with proper OCIO `processSpace='ACEScct'` |
| — | feat: `_getOrCreateOCIOConfig()` — dynamic OCIO config from built-in ACES studio config + shot_grade Look |
| — | feat: Pipeline swap: `RVLookLUT` → `OCIOLook` in `RVLookPipelineGroup` |
| — | feat: Transform chain: ACEScg → ACEScct → .cube → ACEScct → ACEScg (matches Nuke exactly) |
| `a772f54` | chore: bump version to 1.8.7 |
